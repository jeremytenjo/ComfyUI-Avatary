// @ts-nocheck
// Switch Avatary - slot management and state helpers.
//
...
import { ROW_H, TOP_PAD } from "./render.js";
...
export function normalizeSlots(node) {
	...
	if (state.visibleCount !== node.inputs.length)
		state.visibleCount = node.inputs.length;
...
export function updateOutputType(node) {
	...
	const hasActiveLink =
		state.activeIndex >= 1 &&
		node.inputs?.[state.activeIndex - 1]?.link != null;
	const upType = hasActiveLink
		? getUpstreamType(node, state.activeIndex)
		: null;
	if (upType) {
