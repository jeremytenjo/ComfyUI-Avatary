...
// Switch Avatary - dynamic N-to-1 switch with per-row toggles.
...
app.registerExtension({
  ...
  async beforeRegisterNodeDef(nodeType, nodeData) {
    if (nodeData.name !== "AvatarySwitch") return;
...
function buildSwitchNodeIndex() {
  ...
  const visit = (graph) => {
    ...
    for (const n of nodes) {
      if (!n) continue;
      if (n.comfyClass === "AvatarySwitch" || n.type === "AvatarySwitch") {
...
app.graphToPrompt = async function (...args) {
  ...
  if (out) {
    ...
    for (const id in out) {
      const entry = out[id];
      if (!entry || entry.class_type !== "AvatarySwitch") continue;
